...
- [x] 위 4개 항목의 수용 기준 전부 통과 (2026-07-07, 브라우저에서 검증)
- [x] 기존 examples/ 2개가 수정 없이 동작
- [x] ARCHITECTURE.md의 "계획된 ..." 절을 "현재 구현" 절로 옮겨 갱신
- [x] ROADMAP.md 체크박스 갱신

구현 노트 (후속 작업자 참고):
- 1.2는 dict 기반 `_props` 대신 **실제 인스턴스 속성** 저장으로 구현 (읽기 성능 5배 차이,
  ARCHITECTURE.md "속성 동기화 모델" 참고). 스펙의 `_props` 설계는 이 방식으로 대체됨.
- 1.3 스트레스 테스트 측정치: 500구 × 2성분/틱 = 18.0ms/틱. 남은 flush 최적화 레버는
  ARCHITECTURE.md "성능 예산" 참고.
- 추가 수정: 무한 루프 중 Run 재클릭 불가 버그 (Run 버튼을 비활성화하지 않고
  main.js의 runCounter로 상태 표시 경합 정리).
