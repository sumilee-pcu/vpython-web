...
## Phase 1 — 렌더링 코어 보강 (모든 후속 작업의 기반) — ✅ 완료 (2026-07-07)

지금 구조의 한계를 먼저 풀어야 나머지 객체 추가가 쉬워진다.

- ✅ **`axis` / `up` 지원** — 쿼터니언 회전 + box의 size.x↔mag(axis) 결합,
      length/height/width 별칭까지. (spec 1.1 수용 기준 통과)
- ✅ **vector 성분 수정 동기화** — `ball.pos.x += 1` 화면 반영, 대입은 복사 시맨틱.
      (spec 1.2 수용 기준 통과)
- ✅ **업데이트 배칭** — 프로토콜 v2 `vpw.applyUpdates`. 구 500개 스트레스 18ms/틱.
      (spec 1.3 수용 기준 통과)
- ✅ **에러 트레이스백 줄번호 보정** — tokenize 기반 변환(문자열/주석 안전) +
      "N행: 에러" 표시 + 에디터 줄 하이라이트. (spec 1.4 수용 기준 통과)
- 덤: 무한 루프 실행 중 Run 재클릭이 불가능하던 MVP 버그 수정 (Run 상시 활성 + runCounter)
...
- [x] 예제 갤러리 (examples/ 폴더를 드롭다운으로 로드) — 2026-07-09 선구현
